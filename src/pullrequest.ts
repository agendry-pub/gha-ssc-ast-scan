...
async function getSelfCheckRunId(): Promise<number> {
  ...
  if (data.total_count) {
    ...
    throw new Error(
      `Failed to fetch self job id for run ${github.context.runId} [${github.context.issue.owner}:${github.context.issue.repo}]`
    )
  }
}
...
export async function waitForPullRunsCompleted(): Promise<boolean> {
  ...
  const { data: commits } = await octokit.rest.pulls
    .listCommits({
      owner: github.context.issue.owner,
      repo: github.context.issue.repo,
    ...
    .catch((error: any) => {
      ...
      throw new Error(
        `Failed to fetch commit list for pull #${github.context.issue.number} from ${github.context.issue.owner}/${github.context.repo.repo}`
      )
    })
  ...
  await Promise.all(
    commits.map(async commit => {
      const { data: checkRuns } = await octokit.request(
        'GET /repos/{owner}/{repo}/commits/{ref}/check-runs?check_name={check_name}',
        {
          owner: github.context.issue.owner,
          repo: github.context.issue.repo,
          ref: commit.sha,
      ...
      await Promise.all(
        checkRuns.check_runs.map(async function (checkRun: any) {
          if (checkRun.id != selfJobId) {
            ...
            while (
              [
                ...
                'waiting',
                'pending'
              ].includes(checkRunStatus)
            ) {
              core.info(
                `Waiting for Run : [${checkRun.id}] ${checkRun.name}:${
                  commit.commit.message
                } [${utils.shortSha(
                  commit.sha
                )}] to be completed. Curent status: ${checkRun.status}`
              )
              await new Promise(resolve =>
                setTimeout(
                  resolve,
                  Number(
                    utils.getEnvOrValue('GHA_COMMIT_CHECKS_PULL_INTERVAL', 60)
                  ) * 1000
                )
              )

              const { data: tmp } = await octokit.request(
            ...
            core.info(
              `[${checkRun.id}] ${checkRun.name}: ${
                commit.commit.message
              } [${utils.shortSha(commit.sha)}] is ${checkRunStatus} `
            )
          }
        })
      )
...
export async function decorate(appVersionId: string | number): Promise<any> {
  core.info(
    `Decorating pull request #${github.context.issue.number} from ${github.context.issue.owner}/${github.context.repo.repo}`
  )
        ...
        await Promise.all(
          files.map(async function (file: any) {
            ...
            while ((m = regex.exec(file.patch)) !== null) {
              ...
              utils.debugObject(
                `diff: ${file.filename} ${diffHunk.start}:${diffHunk.end}`
              )

              const query: string = `[analysis type]:"sca" AND file:"${file.filename}" AND line:[${diffHunk.start},${diffHunk.end}] AND commit:${commit.sha}`
              let vulns = await vuln.getAppVersionVulns(
                appVersionId,
                query,
                'id'
              )

              await vuln.addDetails(
                vulns,
                'issueName,traceNodes,fullFileName,shortFileName,brief,friority,lineNumber'
              )
              ...
              vulns.forEach(vuln => {
                ...
                const appVersionUrl: string = `${core.getInput(
                  'ssc_base_url'
                )}/html/ssc/version/370/audit?q=${vuln?.issueInstanceId}`
                comments.push({
                  path: file.filename,
        ...
        if (comments.length) {
          ...
          await octokit
            .request('POST /repos/{owner}/{repo}/pulls/{pull_number}/reviews', {
              ...
              body: `Fortify found potential problems in commit ${utils.shortSha(
                commit.sha
              )}`,
              event: 'COMMENT',
              comments: comments,
