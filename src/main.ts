...
const INPUT = {
  ...
  ssc_source_copy_vulns: core.getInput('ssc_source_copy_vulns', {
    required: false
  }),
  ssc_commit_customtag_guid: core.getInput('ssc_commit_customtag_guid', {
    required: true
  }),
  ...
  security_gate_action: core.getInput('security_gate_action', {
    required: false
  }),
  security_gate_filterset: core.getInput('security_gate_filterset', {
    required: false
  }),
...
export async function run(): Promise<void> {
  try {
    ...
    if (github.context.eventName === 'pull_request') {
      ...
      const completed: boolean | void = await pullrequest
        .waitForPullRunsCompleted()
        .catch(error => {
          core.warning(
            `Something went wrong while waiting for PR's related commits check runs to complete: ${error.message}`
          )
        })
      if (completed) {
    ...
    const appVersionId = await appversion.getOrCreateAppVersionId(
      INPUT.ssc_app,
      INPUT.ssc_version,
    ...
    if (INPUT.sast_scan) {
      ...
      await sast
        .packageSourceCode(INPUT.sast_build_options, packagePath)
        .then(packaged => {
          if (packaged != 0) {
            throw new Error(
              utils.failure(
                `Packaging source code with "${INPUT.sast_build_options}"`
              )
            )
          }
        })
      ...
      const jobToken: string = await sast
        .startSastScan(packagePath)
        .catch(error => {
          core.error(error.message)
      ...
      await sast
        .waitForSastScan(jobToken)
        .then(result => {
          if (!result) {
            throw new Error(
              utils.failure(`SAST scan execution (jobToken: ${jobToken})`)
            )
          } else {
            core.info(
              utils.success(`SAST scan execution (jobToken: ${jobToken})`)
            )
          }
        })
        .catch(error => {
      ...
      const fprPath = await artifact.downloadArtifact(jobToken).catch(error => {
        core.error(error.message)
        core.setFailed(
          utils.failure(`Artifact Download (jobToken: ${jobToken})`)
        )
        process.exit(core.ExitCode.Failure)
      })
      ...
      core.info(
        `Artifact Upload to ${INPUT.ssc_app}:${INPUT.ssc_version} [${appVersionId}]`
      )
      const artifactId = await artifact
        .uploadArtifact(appVersionId, fprPath)
        .catch(error => {
          core.error(error.message)
          core.setFailed(
            utils.failure(
      ...
      const scan = await artifact
        .waitForArtifactUpload(artifactId)
        .catch(error => {
          core.error(error.message)
      ...
      try {
        ...
        if (scanVulns.length) {
          ...
          core.info(
            `Checking if ${INPUT.ssc_app}:${INPUT.ssc_version} [${appVersionId}] has Commit CustomTag (guid: ${customTagGuid})`
          )
          if (
            !(await appversion
              .appVersionHasCustomTag(appVersionId, customTagGuid)
              .catch(error => {
                core.error(
                  utils.failure(
                    `Checking if ${INPUT.ssc_app}:${INPUT.ssc_version} [${appVersionId}] has Commit CustomTag (guid: ${customTagGuid})`
            ...
            core.info(
              `AppVersion ${INPUT.ssc_app}:${
                INPUT.ssc_version
              } [${appVersionId}] ${utils.bgYellow(
                'does not have Commit CustomTag'
              )} (guid: ${customTagGuid})`
            )
            core.info(
              `Checking if CustomTag exists in Templates (guid: ${customTagGuid})`
            )
            if (
              await customtag
                .commitCustomTagExists(customTagGuid)
                .catch(error => {
                  core.error(
                    utils.failure(
                      `Checking if CustomTag exists in Templates (guid: ${customTagGuid})`
              ...
              core.info(
                `Adding CustomTag ${customTagGuid} to ${INPUT.ssc_app}:${INPUT.ssc_version} [${appVersionId}]`
              )
              await appversion
                .addCustomTag(appVersionId, customTagGuid)
                .catch(error => {
                  core.error(
                    utils.failure(
                      `Adding CustomTag ${customTagGuid} to ${INPUT.ssc_app}:${INPUT.ssc_version} [${appVersionId}]`
          ...
          core.notice(
            `Current Fortify scan found no ${utils.bgBlue('NEW')} vulnerability`
          )
          core.info(
            utils.skipped(
              `Tagging Vulns with commit SHA (${github.context.sha})`
            )
          )
        }
      } catch (error: any) {
        core.error(
          utils.failure(`Tagging Vulns with commit SHA (${github.context.sha})`)
        )
      }
    }
    if (github.context.eventName === 'pull_request') {
    ...
    const passedSecurityGate = await securitygate
      .run(
        appVersionId,
        INPUT.security_gate_filterset,
        INPUT.security_gate_action
      )
      .catch(error => {
        core.error(error.message)
