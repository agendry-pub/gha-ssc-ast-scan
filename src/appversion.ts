...
export async function getAppVersion(
  ...
  return (
    await utils.fcli([
      ...
      'list',
      `-q`,
      `application.name=='${app}'&&name=='${version}'`,
      '--output=json'
    ])
  )[0]
}
...
async function getAppVersionCustomTags(
...
async function copyAppVersionAudit(
  source: string | number,
  target: string | number
): Promise<boolean> {
  ...
  const customTags = await getAppVersionCustomTags(
    source,
  ...
  const vulns = await vuln.getAppVersionVulns(
    source,
    '',
    '',
...
async function createAppVersion(app: any, version: string): Promise<any> {
  ...
  if (appId > 0) {
    ...
    utils.debugObject(
      `Application ${app} not found. Creating new Application as well`
    )
...
async function runAppVersionCreation(
  app: string,
  version: string,
  source_app?: string,
  ...
  const appVersion = await createAppVersion(app, version).catch(
    async function (error) {
      core.error(`${error.message}`)
      throw new Error(
        utils.failure(`ApplicationVersion ${app}:${version} creation`)
      )
    }
  )
  core.info(utils.success(`ApplicationVersion ${app}:${version} creation`))
  core.info(
    `AppVersion ${appVersion.project.name}:${appVersion.name} created with id: ${appVersion.id}`
  )
  ...
  if (source_version) {
    ...
    core.info(
      `Copy state from ${source_app}:${source_version} to ${app}:${version}`
    )
  ...
  await setAppVersionIssueTemplate(
    appVersion.id,
    core.getInput('ssc_version_issue_template')
  )
    .then(() =>
      core.info(
        utils.success(
          `Setting AppVersion's Issue Template to ${app}:${version}`
        )
      )
    )
    .catch(error => {
  ...
  await setAppVersionAttributes(
    appVersion.id,
    core.getMultilineInput('ssc_version_attributes')
  )
    .then(() =>
      core.info(
        utils.success(`Setting AppVersion's Attributes to ${app}:${version}`)
      )
    )
    .catch(error => {
      core.warning(`${error.message}`)
  ...
  await commitAppVersion(appVersion.id)
    ...
    .catch(async function (error) {
      ...
      await deleteAppVersion(appVersion.id).catch(error => {
        core.error(
          `Failed to delete uncommited version ${appVersion.project.name}:${appVersion.name} [id: ${appVersion.id}`
        )
      })
      throw new Error(
  ...
  if (core.getInput('ssc_source_copy_vulns') && sourceAppVersionId) {
    core.info(
      `Copy Vulnerabilities from ${source_app}:${source_version} to ${app}:${version}`
    )
    if (await copyAppVersionVulns(sourceAppVersionId, appVersion.id)) {
      ...
      core.info(
        `Copy Audit from ${source_app}:${source_version} to ${app}:${version}`
      )
...
export async function getOrCreateAppVersionId(
  app: string,
  version: string,
  source_app?: string,
  source_version?: string
  ...
  let appVersionId = await getAppVersionId(app, version).catch(error => {
    core.error(`${error.message}`)
  ...
  if (appVersionId === undefined || appVersionId === -1) {
    ...
    appVersionId = await runAppVersionCreation(
      app,
      version,
      source_app,
      source_version
...
export async function appVersionHasCustomTag(
  AppVersionId: string | number,
  customTagGuid: string
): Promise<boolean> {
  ...
  return (
    (
      await utils.fcli([
        ...
        `--output=json`,
        `-q`,
        `customTags.![guid].contains('${customTagGuid}')`
      ])
    ).length > 0
  )
}
