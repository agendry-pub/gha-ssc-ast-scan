...
export async function getAppVersionVulnsCount(
  appId: number | string,
  filterSet: string,
  analysisType?: String,
  newIssues?: boolean
  ...
  const url = `/api/v1/projectVersions/${appId}/issueGroups?filterset=${await filterset.getFilterSetGuid(
    appId,
    filterSet
  )}&groupingtype=FOLDER${
    query.length ? `&qm=issues&q=${encodeURI(query)}` : ''
  }`

  return await utils.fcliRest(url)
}

export async function getAppVersionNewVulnsCount(
  appId: number | string,
  filterSet: string,
  analysisType?: String
): Promise<any> {
...
export async function getAppVersionVulnsCountTotal(
  appId: number | string,
  filterSet: string,
  analysisType?: String,
  newIssues: boolean = false
): Promise<any> {
  const count: any[] = await getAppVersionVulnsCount(
    appId,
    filterSet,
    analysisType,
    newIssues
...
export async function getVulnsByScanId(
  appVersionId: number | string,
  ...
  return await getAppVersionVulns(
    appVersionId,
    restQuery,
    `lastScanId==${scanId}`,
    fields
...
export async function getNewVulnsByScanId(
  appVersionId: number | string,
  scanId: number | string
): Promise<any> {
  return await getVulnsByScanId(
    appVersionId,
    scanId,
    'id,revision,lastScanId',
    true
  )
}
...
export async function tagVulns(
  appId: string | number,
  vulns: any[],
  guid: string,
  value: string
  ...
  return (
    (
      await utils.fcliRest(
        `/api/v1/projectVersions/${appId}/issues/action/updateTag`,
        'POST',
        JSON.stringify(body)
      )
    ).length > 0
...
export async function transposeToAppVersion(
  ...
  const targetVulns = await getAppVersionVulns(
    appVersionId,
    '',
    '',
...
export async function auditVulns(
  appVersionId: string | number,
  vulns: any[],
  customTagAudits: any[]
) {
  ...
  return await utils.fcliRest(
    `/api/v1/projectVersions/${appVersionId}/issues/action/audit`,
    'POST',
    JSON.stringify(body).replace('customTagIndex', 'newCustomTagIndex')
  )
}
